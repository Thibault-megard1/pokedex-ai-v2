            ...
            <div className="card p-4">
              <div className="flex flex-wrap items-center justify-between gap-3">
                <div className="text-sm text-gray-700">
                  ...
                  <b>{res.winner === "A" ? getDisplayName(res.a.name, res.a.frenchName) : getDisplayName(res.b.name, res.b.frenchName)}</b>
                </div>
                {chanceText ? (
                  <div className="text-sm">
                    Chances: <b>{getDisplayName(res.a.name, res.a.frenchName)}</b> {chanceText.pctA}% —{" "}
                    <b>{getDisplayName(res.b.name, res.b.frenchName)}</b> {chanceText.pctB}%
                  </div>
                ) : null}
              </div>
            </div>
            ...
            <div className="card p-4 relative overflow-hidden">
              ...
              <div className="flex justify-end">
                <div className="bg-white/85 rounded-xl border px-4 py-3 w-[280px]">
                  <div className="flex items-center justify-between">
                    <div className="font-semibold">{getDisplayName(res.b.name, res.b.frenchName)}</div>
              ...
              <div className="h-[260px] relative">
                ...
                <div className={`absolute top-6 right-10 ${bAnim} bob`}>
                  ...
                  <div className="absolute -top-6 right-0 bg-white/85 border rounded-lg px-2 py-1 text-sm font-semibold">
                    {getDisplayName(res.b.name, res.b.frenchName)}
                  </div>
                  {/* Sprite frontal de l'adversaire - applique la classe 'ko' si KO */}
                  <img className={`poke-sprite w-[140px] h-[140px] ${bKO ? "ko" : ""}`} src={res.b.sprite ?? ""} alt={getDisplayName(res.b.name, res.b.frenchName)} />
                </div>
                ...
                <div className={`absolute bottom-1 left-10 ${aAnim} bob`}>
                  ...
                  <div className="absolute -top-6 left-0 bg-white/85 border rounded-lg px-2 py-1 text-sm font-semibold">
                    {getDisplayName(res.a.name, res.a.frenchName)}
                  </div>
                  {/* Sprite de dos du joueur (ou sprite frontal en fallback) - applique la classe 'ko' si KO */}
                  <img className={`poke-sprite w-[160px] h-[160px] ${aKO ? "ko" : ""}`} src={res.a.backSprite ?? res.a.sprite ?? ""} alt={getDisplayName(res.a.name, res.a.frenchName)} />
                </div>
              </div>
              ...
              <div className="flex justify-start">
                <div className="bg-white/85 rounded-xl border px-4 py-3 w-[320px]">
                  <div className="flex items-center justify-between">
                    <div className="font-semibold">{getDisplayName(res.a.name, res.a.frenchName)}</div>
